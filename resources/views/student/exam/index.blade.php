                        ...
                        @foreach($exams as $exam)
                            <div class="bg-gray-50 p-6 rounded-lg shadow-md">
                                ...
                                @php
                                    ...
                                    if ($exam->name === 'TPA' && !$canTakeTPA) { // Perubahan di sini
                                        $isDisabled = true;
                                        $buttonText = 'Isi KEDUA Jurusan Dulu'; // Perubahan pesan
                                        $buttonClass = 'bg-gray-400 cursor-not-allowed';
                                    } elseif ($currentStudentExam) {
                                        if ($currentStudentExam->status === 'completed') {
                                            $buttonText = 'Lihat Hasil';
                                            $buttonClass = 'bg-blue-500 hover:bg-blue-600';
                                            $actionRoute = route('student.exam.result', $currentStudentExam);
